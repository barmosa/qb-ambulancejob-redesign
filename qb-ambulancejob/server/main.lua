...
-- Callbacks
QBCore.Functions.CreateCallback('hospital:server:getEmsCount', function(source, cb)
    local emsCount = 0
...
-- Initialize player metadata when they first join
AddEventHandler('QBCore:Server:PlayerLoaded', function(Player)
    -- Initialize laststand metadata if not exists
    if Player.PlayerData.metadata['inlaststand'] == nil then
        Player.Functions.SetMetaData('inlaststand', false)
    end
    if Player.PlayerData.metadata['laststandtime'] == nil then
        Player.Functions.SetMetaData('laststandtime', 0)
    end
    if Player.PlayerData.metadata['isincritical'] == nil then
        Player.Functions.SetMetaData('isincritical', false)
    end
end)
...
RegisterNetEvent('hospital:server:SetLaststandTime', function(time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.SetMetaData('laststandtime', time)
end)

RegisterNetEvent('hospital:server:GetLaststandTime', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    local time = Player.PlayerData.metadata.laststandtime or 0
    TriggerClientEvent('hospital:client:SyncLaststandTime', src, time)
end)

RegisterNetEvent('hospital:server:SetCriticalState', function(status)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Player.Functions.SetMetaData('isincritical', status)
end)
