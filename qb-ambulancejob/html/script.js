...
function handleMessage(event) {
    ...
    switch (data.action) {
        case 'hide':
            ...
            $('#ems-instruction').removeClass('hidden ems-waiting').html(`
                <span class="initial-text">Click </span><span class="key" id="e-key">E</span> <span id="ems-text">to call EMS</span>
            `);
            $('#status-text').html('You are <span class="injured">injured</span>');
            $('#timer').removeClass('critical').text('00:00');
        ...
        case 'resetUI':
            ...
            $('#timer').text('00:00');
            $('#ems-instruction').removeClass('hidden ems-waiting');
...
function updateEmsTimer() {
    if (!state.emsCooldown || state.emsTimeRemaining <= 0) {
        ...
        // Reset UI when timer reaches 0
        $('#ems-instruction').html(`
            <span class="initial-text">Click </span><span class="key" id="e-key">E</span> <span id="ems-text">to call EMS</span>
        `).removeClass('ems-waiting');
        state.emsCooldown = false;
        return;
    }
