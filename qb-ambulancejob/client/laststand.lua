...
function SetLaststand(bool)
    ...
    if not bool then
        ...
        -- Get time from metadata if exists
        if Player and Player.metadata and Player.metadata.laststandTime and Player.metadata.laststandTime > 0 then
            LaststandTime = Player.metadata.laststandTime
            isInCriticalState = Player.metadata.isincritical or false
        elseif LaststandTime <= 0 then
            -- Only set new time if we don't have any time (new laststand)
        ...
        -- Wait before sending timer state
        Wait(500)
        ...
        return -- Don't trigger SetLaststandStatus again
    end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    if Player.metadata.inlaststand then
        -- Get saved laststand time from metadata (using laststandTime with capital T)
        LaststandTime = Player.metadata.laststandTime or 0
        isInCriticalState = Player.metadata.isincritical or false

        -- Start the timer if we're in laststand and not critical
        if LaststandTime > 0 and not isInCriticalState then
            StartLaststandTimer()
        end

        -- Wait before setting laststand state
        Wait(1000)
        SetLaststand(true)
