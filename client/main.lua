...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    if not Config.UseBeds then
        -- Fast spawn at specific coordinates
        local ped = PlayerPedId()
        SetEntityCoords(ped, Config.FastSpawn.x, Config.FastSpawn.y, Config.FastSpawn.z)
        SetEntityHeading(ped, Config.FastSpawn.w)
        TriggerEvent('hospital:client:Revive')
        return
    end
